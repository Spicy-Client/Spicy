...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public boolean isEntityInsideOpaqueBlock() {
        final EntityInsideBlockRenderEvent insideBlockRenderEvent = new EntityInsideBlockRenderEvent();
        EventManager.call(insideBlockRenderEvent);
    ...
    public void moveEntity(final double x, final double y, final double z) {
        final PlayerMoveEvent event = new PlayerMoveEvent(x, y, z);
        EventManager.call(event);
        super.moveEntity(event.getX(), event.getY(), event.getZ());
    }

    public void setInWater(final boolean water) {
        this.inWater = water;
    }
    ...
    {
        final PlayerMotionUpdateEvent pre = new PlayerMotionUpdateEvent(this.rotationYaw, this.rotationPitch, this.posX, this.posY, this.posZ, this.onGround, PlayerMotionUpdateEvent.State.PRE);
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.ridingEntity == null)
            {
                if (flag2 && flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(pre.getX(), pre.getY(), pre.getZ(), pre.getYaw(), pre.getPitch(), pre.isOnGround()));
                }
                else if (flag2)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(pre.getX(), pre.getY(), pre.getZ(), pre.isOnGround()));
                }
                else if (flag3)
                {
        ...
        final PlayerMotionUpdateEvent post = new PlayerMotionUpdateEvent(this.rotationYaw, this.rotationPitch, this.posX, this.posY, this.posZ, this.onGround, PlayerMotionUpdateEvent.State.POST);
    ...
    {
        if(Wrapper.onSendChatMessage(message)) {
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z)
    {
        ...
        final EntityPushOutOfBlocksEvent pushOutOfBlockEvent = new EntityPushOutOfBlocksEvent();
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isUsingItem() && !this.isRiding() && !ModuleManager.getModule(NoSlowdown.class).isModuleState())
        {

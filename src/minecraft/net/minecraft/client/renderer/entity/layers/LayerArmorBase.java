...
import spicy.module.modules.player.BetterView;
import spicy.module.modules.render.EnchantColor;
...
public abstract class LayerArmorBase<T extends ModelBase> implements LayerRenderer<EntityLivingBase>
{
    ...
    private void renderLayer(EntityLivingBase entitylivingbaseIn, float p_177182_2_, float p_177182_3_, float partialTicks, float p_177182_5_, float p_177182_6_, float p_177182_7_, float scale, int armorSlot)
    {
        ...
        {
            if (ModuleManager.getModule(BetterView.class).isModuleState() && Minecraft.getMinecraft().gameSettings.thirdPersonView == 1 && entitylivingbaseIn instanceof EntityPlayerSP && (itemstack.getItem() == Items.diamond_helmet || itemstack.getItem() == Items.iron_helmet || itemstack.getItem() == Items.golden_helmet || itemstack.getItem() == Items.leather_helmet || itemstack.getItem() == Items.chainmail_helmet)) {
                return;
            }
    ...
    private void renderGlint(EntityLivingBase entitylivingbaseIn, T modelbaseIn, float p_177183_3_, float p_177183_4_, float partialTicks, float p_177183_6_, float p_177183_7_, float p_177183_8_, float scale)
    {
        if (!Config.isShaders() || !Shaders.isShadowPass)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                float f2 = 0.76F;
                if(ModuleManager.getModule(EnchantColor.class).isModuleState()) {
                    GlStateManager.color(0.5F * f2, 255 * f2, 255 * f2, 255);
                } else {

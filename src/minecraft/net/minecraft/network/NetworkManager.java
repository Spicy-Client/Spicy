...
public class NetworkManager extends SimpleChannelInboundHandler<Packet>
{
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) throws Exception
    {
        if (this.channel.isOpen())
        {
            try
            {
                final TakePacketEvent pre = new TakePacketEvent(p_channelRead0_2_, TakePacketEvent.PacketState.PRE);
                EventManager.call(pre);
                if (pre.isCancelled()) {
                    return;
                }
                p_channelRead0_2_.processPacket(this.packetListener);
                final TakePacketEvent post = new TakePacketEvent(p_channelRead0_2_, TakePacketEvent.PacketState.POST);
    ...
    {
        final SendPacketEvent pre = new SendPacketEvent(inPacket, SendPacketEvent.PacketState.PRE);
        EventManager.call(pre);
        if (pre.isCancelled()) {
            return;
        }
        final EnumConnectionState enumconnectionstate = EnumConnectionState.getFromPacket(pre.getPacket());
        ...
        final SendPacketEvent post = new SendPacketEvent(inPacket, SendPacketEvent.PacketState.POST);
        EventManager.call(post);
    }
    ...
    public void sendNewPacket(final Packet packetIn) {
        if (this.channel != null && this.channel.isOpen()) {

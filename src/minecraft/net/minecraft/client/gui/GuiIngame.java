...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        EventManager.call(new Render2DEvent(i, j));
        if (this.recordPlayingUpFor > 0)
        {
            ...
            if (l1 > 8)
            {
                ...
                Spicy.INSTANCE.fontManager.getFont("FONT 20").drawStringWithShadow(this.recordPlaying.replaceAll("ı", "i").replaceAll("ğ", "g").replaceAll("Ğ", "G").replaceAll("İ", "I").replaceAll("ş", "s").replaceAll("Ş", "S"), -Spicy.INSTANCE.fontManager.getFont("FONT 20").getWidth(this.recordPlaying) / 2, -4, l + (l1 << 24 & -16777216));
                GlStateManager.disableBlend();
    ...
    public void renderSelectedItem(ScaledResolution scaledRes)
    {
        ...
        if (this.remainingHighlightTicks > 0 && this.highlightingItemStack != null)
        {
            ...
            if (k > 0)
            {
                ...
                GlStateManager.enableBlend();
                GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
                Spicy.INSTANCE.fontManager.getFont("FONT 15").drawStringWithShadow(s.replaceAll("ı", "i").replaceAll("ğ", "g").replaceAll("Ğ", "G").replaceAll("İ", "I").replaceAll("ş", "s").replaceAll("Ş", "S"), (float)i, (float)j, 16777215 + (k << 24));
                GlStateManager.disableBlend();
                GlStateManager.popMatrix();
            }
        }
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        int i = (int) Spicy.INSTANCE.fontManager.getFont("FONT 15").getWidth(objective.getDisplayName());

        for (Score score : collection)
        {
            ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(score.getPlayerName());
            String s = ScorePlayerTeam.formatPlayerName(scoreplayerteam, score.getPlayerName()) + ": " + EnumChatFormatting.WHITE + score.getScorePoints();
            i = (int) Math.max(i, Spicy.INSTANCE.fontManager.getFont("FONT 15").getWidth(s));
        }

        int i1 = (int) (collection.size() * Spicy.INSTANCE.fontManager.getFont("FONT 15").getHeight(String.valueOf(collection.size())));
        int j1 = (scaledRes.getScaledHeight() / 4 + i1 / 2) + IngameUI.yCount;
        int k1 = 3;
        ...
        for (Score score1 : collection)
        {
            ...
            String s2 = EnumChatFormatting.WHITE + "" + score1.getScorePoints();
            int k = j1 - j * this.getFontRenderer().FONT_HEIGHT;
            int l = scaledRes.getScaledWidth() - k1 + 2;
            drawRect(l1 - 2, k, l, k + this.getFontRenderer().FONT_HEIGHT, 1342177280);
            GlStateManager.color(1,1,1);
            Spicy.INSTANCE.fontManager.getFont("FONT 15").drawString(s1.replaceAll("ı", "i").replaceAll("ğ", "g").replaceAll("Ğ", "G").replaceAll("İ", "I").replaceAll("ş", "s").replaceAll("Ş", "S"), l1, k, -1);
            Spicy.INSTANCE.fontManager.getFont("FONT 15").drawString(s2.replaceAll("ı", "i").replaceAll("ğ", "g").replaceAll("Ğ", "G").replaceAll("İ", "I").replaceAll("ş", "s").replaceAll("Ş", "S"), l - Spicy.INSTANCE.fontManager.getFont("FONT 15").getWidth(s2), k, -1);

            if (j == collection.size())
            {
                String s3 = objective.getDisplayName();
                drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - 1, 1342177280);
                drawRect(l1 - 2, k - 1, l, k, 1342177280);
                GlStateManager.color(1,1,1);
                Spicy.INSTANCE.fontManager.getFont("FONT 15").drawString(s3.replaceAll("ı", "i").replaceAll("ğ", "g").replaceAll("Ğ", "G").replaceAll("İ", "I").replaceAll("ş", "s").replaceAll("Ş", "S"), l1 + i / 2 - Spicy.INSTANCE.fontManager.getFont("FONT 15").getWidth(s3) / 2, k - this.getFontRenderer().FONT_HEIGHT, -1);
                GlStateManager.color(1,1,1);
            }
        }
    }
    ...
    private void renderBossHealth()
    {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0)
        {
            ...
            Spicy.INSTANCE.fontManager.getFont("FONT 15").drawStringWithShadow(s, (float)(i / 2 - this.getFontRenderer().getStringWidth(s) / 2), (float)(i1 - 10), 16777215);
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            this.mc.getTextureManager().bindTexture(icons);
        }
    }

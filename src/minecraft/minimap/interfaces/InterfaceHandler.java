package minimap.interfaces;

import minimap.XaeroMinimap;
import minimap.minimap.Minimap;
import minimap.settings.ModOptions;
        ...
        InterfaceHandler.minimap = new Minimap(new Interface("gui.xaero_minimap", 128, 128, ModOptions.MINIMAP) {
            ...
            public void drawTextUnderMinimap(final int scaledX, final int scaledY, final int height, final float mapScale) {
                ...
                for (int i = 0; i < this.underText.size(); ++i) {
                    ...
                    final int stringWidth = (int) Spicy.INSTANCE.fontManager.getFont("FONT 15").getWidth(s);
                    final boolean under = scaledY + interfaceSize / 2 < scaledHeight / 2;
                    final int stringY = scaledY + (under ? interfaceSize : -9) + i * 9 * (under ? 1 : -1);
                    Spicy.INSTANCE.fontManager.getFont("FONT 15").drawStringWithShadow(s, (float)(scaledX + interfaceSize / 2 - stringWidth / 2), (float)stringY, Minimap.radarPlayers.hashCode());
                }
            ...
            private void drawCompass(final int specW, final int specH, final double ps, final double pc) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    Spicy.INSTANCE.fontManager.getFont("FONT 20").drawStringWithShadow(nesw[i], (float)(1 - Spicy.INSTANCE.fontManager.getFont("FONT 20").getWidth(nesw[i]) / 2), -3.0f, -1);

...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry
{
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected)
    {
        ...
        Spicy.INSTANCE.fontManager.getFont("FONT 15").drawStringWithShadow(this.server.serverName.replaceAll("ı", "i").replaceAll("ğ", "g").replaceAll("Ğ", "G").replaceAll("İ", "I").replaceAll("ş", "s").replaceAll("Ş", "S"), x + 32 + 3, y + 1, -1);
        List<String> list = this.mc.fontRendererObj.listFormattedStringToWidth(this.server.serverMOTD, listWidth - 32 - 2);

        for (int i = 0; i < Math.min(list.size(), 2); ++i)
        {
            Spicy.INSTANCE.fontManager.getFont("FONT 15").drawStringWithShadow(list.get(i).replaceAll("ı", "i").replaceAll("ğ", "g").replaceAll("Ğ", "G").replaceAll("İ", "I").replaceAll("ş", "s").replaceAll("Ş", "S"), x + 32 + 3, y + 12 + this.mc.fontRendererObj.FONT_HEIGHT * i, -1);
        }

        String s2 = flag2 ? EnumChatFormatting.DARK_RED + this.server.gameVersion : this.server.populationInfo;
        int j = (int) Spicy.INSTANCE.fontManager.getFont("FONT 15").getWidth(s2);
        Spicy.INSTANCE.fontManager.getFont("FONT 15").drawString(s2.replaceAll("ı", "i").replaceAll("ğ", "g").replaceAll("Ğ", "G").replaceAll("İ", "I").replaceAll("ş", "s").replaceAll("Ş", "S"), x + listWidth - j - 15 - 2, y + 1, -1);
        int k = 0;
        String s = null;
        int l;
        String s1;
